...
class GbdtClassfierClass(VocBaseClass):
    ...
    def train(self):
        ...
        self._model = GradientBoostingClassifier(loss='deviance', learing_rate=0.05, subsample=0.7 , n_estimators=50)
        ...
        joblib.dump(self._model, self._module_dir + '/gbdt_oneRestLr.model', compress=1)
        joblib.dump(self.mlb, self._module_dir + '/gbdt_mlb.model', compress=1)
        
    
    def load_model(self):
        print('start load model ......')
        self._model = joblib.load(self._module_dir + '/gbdt_oneRestLr.model')
        self.mlb = joblib.load(self._module_dir + '/gbdt_mlb.model')       
        if self._fe:
            self._fe.load_model()
    ...
    def classify_batch(self, text_batch):
     ...
     def predict(self,test_file_path, threshold=0.5):
        print('start predict ............')
        predict_file_path = self._module_dir + '/gbdt_predict.csv')
...
def main():
    voc = SvmClassfierClass('./data/mbb_train.csv','./data/gbdt')
    voc.train()
    voc.load_model()
